...
def update_memory_info():
    ...
    while(1):
        ...
        memory["free"] = int(os.popen('free -m  | grep ^Mem | tr -s \' \' | cut -d \' \' -f3').read())
        memory["used"] = int(os.popen('free -m  | grep ^Mem | tr -s \' \' | cut -d \' \' -f4').read())
        time.sleep(1)
...
def update_info(n):
    return [
        ...
        html.Div([
            html.Div('Memória total: ', className='info-container'),
